...
pf_vmaxm=1.4; pf_taom=0.8; pf_gamam=1.0;
ps_vmaxm=2.0; ps_taom=3.2; ps_gamam=2.0;
...
for i=1:size(fs_dir,1)
  ...
  %2: myofibre
        ...
        if p_lamd >= p_lamd_crm	
            if pf_v>=pf_vmaxm
                stateNew(i,2)=pf_vmaxm;
            else
                for k=1:100			
					p_zhi=abs(pf_vmaxm-pf_v)/(pf_vmaxm-1.0);
					p_k=1.0/pf_taom*p_zhi^pf_gamam;
					p_phi=abs(p_lamd-p_lamd_crm);
  ...
  %equivalent growth tensor
        ...
        gv(i,2)=stateNew(i,2)/stateOld(i,2);
        stateOld(i,2)=stateNew(i,2);
        gml=gv(i,2);
        %vf=[1 0 0]';vs=[0 1 0]';
		
        for ia=1:3
            for ib=1:3
                FgM(ia,ib)=gv(i,2)*vf_M(ia)*vf_M(ib)+vs_M(ia)*vs_M(ib)+vn_M(ia)*vn_M(ib);
            end
        end
        %FgM=FgM+eye(3);       
        FgMI=inv(FgM); 
  ...
  %equivalent growth tensor
        ...
        for ia=1:3
            for ib=1:3
                FgG(ia,ib)=gml*vf_M(ia)*vf_M(ib)+1/sqrt(gml)*vs_M(ia)*vs_M(ib)+1/sqrt(gml)*vn_M(ia)*vn_M(ib);
            end
        end
        %FgG=FgG+eye(3);       
        FgGI=inv(FgG); 
  ...
  %equivalent growth tensor
        ...
        for ia=1:3
            for ib=1:3
                %FgC(ia,ib)=gml*vf_C(ia)*vf_C(ib)+1/sqrt(gml)*vs_C(ia)*vs_C(ib)+1/sqrt(gml)*vn_C(ia)*vn_C(ib);
                FgC(ia,ib)=gv(i,3)*vf_M(ia)*vf_M(ib)+vs_M(ia)*vs_M(ib)+vn_M(ia)*vn_M(ib);
            end
        end
        %FgC=FgC+eye(3);
        %lambda_g=gv(i,2);
		...
		Ffwrite(i,4,:)=vn_C;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        newmass=stateSum(i,1)*gv(i,1)+stateSum(i,2)*gv(i,2)+stateSum(i,3)*gv(i,3);
        stateSum(i,1)=stateSum(i,1)*gv(i,1)/newmass;
        stateSum(i,2)=stateSum(i,2)*gv(i,2)/newmass;
