...
for imIndex = 1 : size(DataSegSA, 2)
    ...
    if ~isempty(epi_cpa)
        ...
        DicomeRealDisplay(h3D_ori, imData);
...
%%rotate BCs
    ...
    for imIndex = 1 : size(DataSegSA, 2)  
        ...
        imInfo = infoExtract(imInfo1);
        imData = MRIMapToRealWithImageAndHeadData(imData, imInfo);
        imData_x =  imData.imReal.X;
        imData_y =  imData.imReal.Y;
        imData_z =  imData.imReal.Z;
        A = reshape(imData_x,[],1);
        B = reshape(imData_y,[],1);
        C = reshape(imData_z,[],1);
        imData_xyz=[A B C];
        imData_xyzT(:,1) = imData_xyz(:,1)-LVUpperCenter(1);
        imData_xyzT(:,2) = imData_xyz(:,2)-LVUpperCenter(2);
        imData_xyzT(:,3) = imData_xyz(:,3)-LVUpperCenter(3);
        imData_xyzT = RotationMatrix*imData_xyzT';
        imData_xyz=imData_xyzT';
        imData_x= reshape(imData_xyz(:,1),[],size(imData_x,2));
        imData_y= reshape(imData_xyz(:,2),[],size(imData_x,2));
        imData_z= reshape(imData_xyz(:,3),[],size(imData_x,2));
        imData.imReal.X=imData_x;
        imData.imReal.Y=imData_y;
        imData.imReal.Z=imData_z;

        DicomeRealDisplay(h3D_rotated, imData);
        clear imData_xyzT A B C imData_xyz imData_x imData_y imData_z;
...
divd=100;
g3D_spline = figure(); hold on; axis equal; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        DicomeRealDisplay(g3D_spline, imData);
        clear imData_xyzT A B C imData_xyz imData_x imData_y imData_z;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%% RV-EPI-top
if size(epi_crv_data,1)>3
min_z=min(epi_crv_data(3:3:size(epi_crv_data,1),1));
...
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=epi_crv_data(1:3:size(epi_crv_data),i); 
    y=epi_crv_data(2:3:size(epi_crv_data),i);
    z=epi_crv_data(3:3:size(epi_crv_data),i); 
...
%%%% PA-EPI-top
if size(epi_cpa_data,1)>3
min_z=min(epi_cpa_data(3:3:size(epi_cpa_data,1),1));
...
max_z=min(epi_crv_data(3:3:size(epi_crv_data,1),1));%max(DataSegLARotated(2).epi_cReal(3,:));
z_int= min_z:0.4:max_z; linspace(min_z, max_z, divd/2);
RV_EPI_top=[];
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=[epi_crv_data(size(epi_crv_data,1)-2,i);   epi_c_data(1,i) ;   epi_c_data(4,i)]; 
    y=[epi_crv_data(size(epi_crv_data,1)-1,i);   epi_c_data(2,i);   epi_c_data(5,i)]; 
    z=[epi_crv_data(size(epi_crv_data,1),i);     epi_c_data(3,i);   epi_c_data(6,i)]; 
...
%%%% PA-EPI-top-overlap
min_z=max(epi_c_data(3:3:size(epi_c_data,1),1));
max_z=min(epi_cpa_data(3:3:size(epi_cpa_data,1),1));%max(DataSegLARotated(2).epi_cReal(3,:));
z_int= min_z:0.4:max_z;%linspace(min_z, max_z, divd/2);
PA_EPI_top=[];
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=[epi_cpa_data(1:3:size(epi_cpa_data,1),i); epi_c_data(1,i) ;   epi_c_data(4,i)]; 
    y=[epi_cpa_data(2:3:size(epi_cpa_data,1),i); epi_c_data(2,i) ;   epi_c_data(5,i)]; 
    z=[epi_cpa_data(3:3:size(epi_cpa_data,1),i); epi_c_data(3,i) ;   epi_c_data(6,i)]; 
