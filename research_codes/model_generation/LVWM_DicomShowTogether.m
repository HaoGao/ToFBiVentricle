...
for i=1:divd/2
    ...
    x(x_len+2:x_len*2+1)=endo_lv_data(size(endo_lv_data)-2:-3:1,i+50);
    y(x_len+2:x_len*2+1)=endo_lv_data(size(endo_lv_data)-1:-3:1,i+50);
    z(x_len+2:x_len*2+1)=endo_lv_data(size(endo_lv_data):-3:1,i+50);
    
    x(x_len*2+2)=endo_lv_data(1,i);
    y(x_len*2+2)=endo_lv_data(2,i);
    z(x_len*2+2)=endo_lv_data(3,i);
    ...
    LV_new=[LV_new points];
    
    plot3(points(1,:),points(2,:),points(3,:),'LineStyle', '-', 'Color', ...
                            'b', 'LineWidth',2)
...
max_z=0;%max(DataSegLARotated(2).endo_rvReal(3,:));
...
for i=1:divd/2
    ...
    x(x_len+2:x_len*2+1)=endo_rv_data(size(endo_rv_data)-2:-3:1,i+50);
    y(x_len+2:x_len*2+1)=endo_rv_data(size(endo_rv_data)-1:-3:1,i+50);
    z(x_len+2:x_len*2+1)=endo_rv_data(size(endo_rv_data):-3:1,i+50);
    
    x(x_len*2+2)=endo_rv_data(1,i);
    y(x_len*2+2)=endo_rv_data(2,i);
    z(x_len*2+2)=endo_rv_data(3,i);
...
max_z=0;%max(DataSegLARotated(2).epi_cReal(3,:));
...
for i=1:divd/2
    ...
    x(x_len+2:x_len*2+1)=epi_c_data(size(epi_c_data)-2:-3:1,i+50);
    y(x_len+2:x_len*2+1)=epi_c_data(size(epi_c_data)-1:-3:1,i+50);
    z(x_len+2:x_len*2+1)=epi_c_data(size(epi_c_data):-3:1,i+50);
    
    x(x_len*2+2)=epi_c_data(1,i);
    y(x_len*2+2)=epi_c_data(2,i);
    z(x_len*2+2)=epi_c_data(3,i);

    
    points=[];theta=[];rho=[];
    points = fnplt(cscvn([x y z]'));
    EPI_new=[EPI_new points];
...
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=endo_rv_data(10:3:size(endo_rv_data),i);
    y=endo_rv_data(11:3:size(endo_rv_data),i);
    z=endo_rv_data(12:3:size(endo_rv_data),i);
    x_len=length(x);
    x(x_len+1)=DataSegLARotated(2).endo_rvReal(1,apex_n);
    y(x_len+1)=DataSegLARotated(2).endo_rvReal(2,apex_n);
    z(x_len+1)=DataSegLARotated(2).endo_rvReal(3,apex_n);
...
%%%% RV-top
min_z=0;
max_z=30.5;%max(DataSegLARotated(2).endo_rvReal(3,:));
apex_n=find(DataSegLARotated(2).endo_rvReal(3,:)==min_z);
z_int= linspace(min_z, max_z, divd/4);
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=endo_rv_data(1:3:size(endo_rv_data),i);
...
for i=1:divd
    new_data=[];
    x=[];y=[];z=[];
    x=epi_c_data(10:3:size(epi_c_data),i);
    y=epi_c_data(11:3:size(epi_c_data),i);
    z=epi_c_data(12:3:size(epi_c_data),i);
    x_len=length(x);
    x(x_len+1)=DataSegLARotated(2).epi_cReal(1,apex_n);
    y(x_len+1)=DataSegLARotated(2).epi_cReal(2,apex_n);
    z(x_len+1)=DataSegLARotated(2).epi_cReal(3,apex_n);
...
%%%% EPI-top
min_z=0;
max_z=30.5;%max(DataSegLARotated(2).epi_cReal(3,:));
apex_n=find(DataSegLARotated(2).epi_cReal(3,:)==min_z);
z_int= linspace(min_z, max_z, divd/4);
for i=1:divd
    new_data=[];
