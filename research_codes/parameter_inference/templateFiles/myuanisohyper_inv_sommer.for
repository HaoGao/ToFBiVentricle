	 !DEC$ ATTRIBUTES ALIAS:"uanisohyper_inv"::UANISOHYPER_INV  
C-------------------------------------------------------------
...
C
C
      if (cmname(1:20) .eq. 'UANISO_GG_MYOCARDIUM') then
...
C
C
C     this is based on Guan's paper when fitting Sommer's human data
C     it uses I1, I4f, I4n, I8fs
      return
      end
c------------------------------------------------------------------
c
...
C - I4(22) refer to I4n
      nI433 = indxInv4(3,3)
      bi433 = aInv(nI433)
      aux1n = bi433-one
      aux2n = half+sign(half,aux1n+asmall)
      term1 = max(aux1n,zero)
      term2 = half*an*exp(bn*term1**2)
      ua(2) = ua(2)+(one/bn)*(term2 - half*an)
      ui1(nI433) = two*term1*term2
      ui2(indx(nI433,nI433))= two*term2*(one + two*bn*term1**2)*aux2n
